...
python train.py --niter 500 --niter_decay 250 --unroll_frames 5 --train_mode "mixed" --clip_grads .1 --n_critic 2 --lambda_L1 100 --lambda_S 1 --lambda_T 1 --max_clip_length $LEN --skip_frames $SKIP --validation_freq 5 --validation_set val --pretrain_epochs 1 --tlg .2 --tld .8 --init_type orthogonal --init_gain .0002 --batch_size $BATCHSIZE --resolution $RESOLUTION --fps $FPS --dataroot $DATASETS_DIR/$DATASET --checkpoints_dir $CHECKPOINT_DIR --name $NAME --model $MODEL --dataset_mode $DATASET_MODE --gpu_ids $GPU_ID --lr $LR --display_port $VISDOM_PORT --display_env $NAME --update_html_freq $FREQ --print_freq $FREQ --display_freq $FREQ
