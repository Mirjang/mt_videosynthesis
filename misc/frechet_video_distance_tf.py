...
def fvd(real_np, fake_np): 
    with tf.Graph().as_default():
        real_videos = tf.convert_to_tensor(real_np)
        generated_videos = tf.convert_to_tensor(fake_np)
...
#     out = embedding.eval()
#     print(f"saving embedding to {os.path.join(root, out_file)}")
#     numpy.save(os.path.join(root, out_file), out)