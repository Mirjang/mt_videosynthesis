...
#NAME=${DATASET}_${MODEL}_${GENERATOR}_${RESOLUTION}_noise_cgan_aux_seg_g32s16t32
git add -A 
git commit -m $NAME
DISPNAME=${NAME}
# vid settings
...
python train.py --ch_g 32 --ch_ds 16 --ch_dt 16 --continue_train --epoch 55 --conditional --lr_policy cosine --niter 10 --niter_decay 250 --clip_grads 2 --n_critic 3 --lambda_L1 50 --lambda_S 1 --lambda_T 50 --lambda_AUX 1 --max_clip_length $LEN --skip_frames $SKIP --validation_freq 5 --validation_set $VAL_SET --pretrain_epochs 0 --tlg .3 --tld .7 --max_val_dataset_size $VAL_SIZE --init_type orthogonal --batch_size $BATCHSIZE --parallell_batch_size $SUB_BATCH --resolution $RESOLUTION --fps $FPS --dataroot $DATASETS_DIR/$DATASET --checkpoints_dir $CHECKPOINT_DIR --name $NAME --model $MODEL --generator $GENERATOR --dataset_mode $DATASET_MODE --gpu_ids $GPU_ID --lr $LR --display_port $VISDOM_PORT --display_env $DISPNAME --update_html_freq $FREQ --num_display_frames $DISP_FRAMES --print_freq $FREQ --display_freq $FREQ
